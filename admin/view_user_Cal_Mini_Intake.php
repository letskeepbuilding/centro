...
session_start();
        ...
        $userData = $db->prepare("SELECT `CAL-Client-Mini-Intake`.user_id, `users`.name, `users`.lastname, `users`.email, `users`.address, `CAL-Client-Mini-Intake`.date_created FROM `CAL-Client-Mini-Intake` INNER JOIN `users` ON `CAL-Client-Mini-Intake`.user_id = `users`.id");
        $userData->execute();
...
<script>
   ...
   var table=     $('#userTable').dataTable( {
            ...
            "sServerMethod": "GET",
            "aaSorting": [[ 0, "desc" ]],
            "sAjaxSource": "code/ajaxUserView_Cal_Mini_Intake.php",
            "aoColumns": [
